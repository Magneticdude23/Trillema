...
**[▶ Play it here](https://Magneticdude23.github.io/Trillema/)**
...
Every formula running under the hood - the IS/Phillips/Okun core, UIP, PPP, the fiscal multiplier, and the public-debt identity - is derived and documented on the **[model spec page](https://Magneticdude23.github.io/Trillema/model-reference.html)**, alongside the exact discretized equation the code runs each tick.
...
model-reference.css      spec page styles
game.js                  simulation engine + UI
mathjax-config.js        MathJax setup
```

The simulation engine lives in `game.js` - the `step()` function is the core, containing the IS curve, Phillips curve, Okun's law, UIP capital flows, PPP exchange-rate dynamics, and the debt identity, all integrated with Euler–Maruyama.
